...
class GaussianNoiseSyntheticDataset(TabularDataset):
        ...
        **kwargs,
        ...
        if connectivity is not None:
            self.connectivity = parse_connectivity(
                connectivity, target_layout="edge_index", num_nodes=num_nodes
            )
        else:
    ...
    def _model_forward(self, *args, **kwargs):
        ...
        # it will contain [output, hidden_state].
        return out
    ...
    def generate_data(self, seed=None):
        ...
        x = (
            torch.empty(
                (
                    self._num_steps + self._min_window,
                    self._num_nodes,
                    self._num_features,
                )
        ...
        y_opt = torch.empty((self._num_steps, self._num_nodes, self._num_features))
        ...
        with torch.no_grad():
            h_t = None
            for t in tqdm(
                range(self._min_window, self._min_window + self._num_steps),
                desc=f"Generating {self.__class__.__name__} data",
            ):
                x_t, h_t = self._model_forward(
                    x[None, t - self._min_window : t],
                    h=h_t,
                    t=t,
                ...
                x_t = (
                    x_t
                    + torch.zeros_like(x_t).normal_(generator=rng) * self.sigma_noise
                )
                x[t : t + 1] = x_t[0]
    ...
    def get_connectivity(self, layout: str = "edge_index"):
        ...
        return parse_connectivity(
            connectivity=self.connectivity, target_layout=layout, num_nodes=self.n_nodes
        )
