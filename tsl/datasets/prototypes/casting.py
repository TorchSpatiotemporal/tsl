...
def to_nodes_channels_columns(df: pd.DataFrame, inplace: bool = True) -> pd.DataFrame:
    if not inplace:
        df = df.copy()
    if df.columns.nlevels == 1:
        nodes = list(df.columns)
        columns = pd.MultiIndex.from_product(
            [nodes, pd.RangeIndex(1)], names=["nodes", "channels"]
        )
        df.columns = columns
        ...
        raise ValueError(
            "Input dataframe must have either 1 ('nodes') "
            "or 2 ('nodes', 'channels') column levels."
        )
    return df


def convert_precision_df(
    df: pd.DataFrame, precision: Union[int, str] = None, inplace: bool = True
) -> pd.DataFrame:
...
def convert_precision_numpy(
    arr: np.ndarray, precision: Union[int, str] = None
) -> np.ndarray:
    if precision is None:
        return arr
    precision = precision_stoi(precision)
...
def check_time_unit(time_unit: str, include_onehot: bool = False):
    ...
    if include_onehot:
        allowed_units.update({"weekday", "day_of_week", "dayofweek", "weekofyear"})
    if time_unit not in allowed_units:
        raise RuntimeError(
            f"'{time_unit}' is not a valid time unit. "
