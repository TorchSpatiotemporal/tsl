...
def _to_undirected_no_selfloops(edge_index: np.ndarray,
                                edge_weight: Optional[Union[np.ndarray, int, float]]
                                ) -> Tuple[np.ndarray, Optional[np.ndarray]]:
    """Remove self-loops and render the graph undirected where the weights
    of duplicated edges are summed. It works with numpy.ndarray."""
    ...
    if edge_weight_ is not None:
        edge_weight_ = np.add.reduceat(edge_weight_, unique_mask_inds, dtype=edge_weight_.dtype)
...
def az_whiteness_test(x: TensArray,
                      ...
                      remove_median: bool = False) -> Union[AZWhitenessTestResult, AZWhitenessMultiTestResult]:
    """Implementation AZ-whiteness test for signal uncorrelation from paper:
    Zambon, Daniele, and Cesare Alippi. "AZ-whiteness test: a test for uncorrelated
    noise on spatio-temporal graphs.", arXiv preprint arXiv:2204.11135 (2022).

    Parameters:
     - `x`: graph signal, typically with pattern "T N F".
     - `mask`: boolean mask of signal `x`, with same size of `x`; the mask is `True`
            where the observations in `x` are valid and `False` otherwise.
     - `pattern`: string encoding the index pattern of `x`, typically "T N F" representing
            time, nodes and node features dimensions, respectively.
     - `edge_index`: indices of the spatial edges with shape (2, E); current implementation
            supports only a static topology.
     - `edge_weight`: positive weights of the spatial edges. It can be a TensArray of
            shape (E,), or a scala value (same weight for all edges).
     - `edge_weight_temporal`: positive scalar weight for all termporal edges; if `None`
            or `"auto"` the weight is computed to balance the contribution of the spatial
            and temporal components (see Zambon and Alippi 2022).
     - `lamb`: scalar factor in within 0.0 and 1.0 defining a convex combination of the
            spatial and temporal components; if `lamb == 1.0` the test is applied on the
            spatial topology only, for `lamb == 0.0` only the serial correlation is
            accounted.
    ...
    az_test_args = dict(x=x, mask=mask, pattern=pattern,
                        edge_index_spatial=edge_index_spatial, edge_weight_spatial=edge_weight_spatial,
...
def _az_whiteness_test(x, mask, pattern,
    ...
    edge_index_spatial, edge_weight_spatial = _to_undirected_no_selfloops(edge_index=edge_index_spatial,
