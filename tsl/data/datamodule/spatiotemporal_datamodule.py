...
class SpatioTemporalDataModule(LightningDataModule):
    ...
    def _add_set(self, split_type, _set):
        ...
        if _set is None or isinstance(_set, Dataset):
            ...
            _slice = self.torch_dataset.expand_indices(_set.indices, merge=True)
            setattr(self, name, _set)
    ...
    def setup(self, stage: StageOptions = None):
        ...
        for key, scaler in self.scalers.items():
            if key not in self.torch_dataset:
                raise RuntimeError(
                    f"Cannot find a tensor to scale matching key '{key}'."
                )
            # set scalers
            if stage in ('validate', 'test', 'predict'):
                ...
                if self.train_slice is None:
                    raise RuntimeError(
                        f"Cannot fit scaler for '{key}': no training slice "
                        "is available. Configure a `splitter` on the "
                        "datamodule, or fit scalers externally and call "
                        "setup(stage=...) with one of "
    ...
    def train_dataloader(
        self, shuffle: bool = True, batch_size: Optional[int] = None
    ) -> Optional[DataLoader]:
        """"""
        return self.get_dataloader('train', shuffle, batch_size)

    def val_dataloader(
        self, shuffle: bool = False, batch_size: Optional[int] = None
    ) -> Optional[DataLoader]:
        """"""
