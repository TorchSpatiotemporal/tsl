...
class SpatioTemporalDataset(Dataset, DataParsingMixin):
    ...
    def patterns(self) -> dict:
        ...
        * 'e' stands for “number edges”
        ...
        if self.edge_index is not None:
            patterns['edge_index'] = '2 e' if self._is_coo_edge_index() else 'n n'
    ...
    def batch_patterns(self) -> dict:
        ...
        * 'e' stands for “number edges”
        ...
        patterns.update(
            {name: attr.pattern for name, attr in self.auxiliary_map.items()}
        )
        return patterns
    ...
    def reset_auxiliary_map(self):
        ...
        if self.mask is not None:
            self.auxiliary_map['mask'] = BatchMapItem(
                'mask',
                SynchMode.HORIZON,
                preprocess=False,
                cat_dim=None,
    ...
    def set_mask(self, mask: Optional[DataArray], add_to_auxiliary_map: bool = True):
    ...
    def update_covariate(
        ...
        if name not in self._covariates:
            raise RuntimeError(
                f"There is not a covariate named {name} in {self.__class__.__name__}"
            )
