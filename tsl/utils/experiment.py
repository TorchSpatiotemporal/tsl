...
class TslExperiment:
    ...
    Args:
        run_fn: Python function that actually runs the experiment when called.
                The run function must accept single argument
                being the experiment hyperparameters.
    ...
    def __init__(self, run_fn, parser: ArgumentParser, debug=False, config_path=None):
        self.run_fn = run_fn
        self.parser = parser
        self.debug = debug
        self.config_root = (
            config_path if config_path is not None else tsl.config.config_dir
        )
    ...
    def run_search_parallel(self, n, workers, gpus=None):
        ...
        if gpus is None:
            ...
            hparams.optimize_parallel_gpu(self.run_fn, max_nb_trials=n, gpu_ids=gpus)
