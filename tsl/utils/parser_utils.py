...
def update_from_config(args: Namespace, config: dict):
    assert set(config.keys()) <= set(
        vars(args)
    ), f"{set(config.keys()).difference(vars(args))} not in args."
...
def parse_by_group(parser):
    ...
    pos_optional_args = {
        name: value for name, value in args_dict if name in pos_optional_arg_names
    }
...
def filter_argparse_args(
    args: Union[Namespace, dict], cls: Type, return_dict: bool = False
):
    ...
    if "add_argparse_args" in dir(cls):
        ...
        raise RuntimeError(
            f"Target class {cls} has not valid method for " f"argparse filtering."
        )
