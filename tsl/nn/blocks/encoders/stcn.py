...
class SpatioTemporalConvNet(nn.Module):
    ...
    Applies several temporal convolutions
    followed by diffusion convolution over a graph.
    ...
    Args:
        ...
        gated (bool, optional): Whether to use the GatedTanH activation function
            after temporal convolutions. (default: `False`)
        pad (bool, optional): Whether to pad the input sequence
            to preserve the sequence length.
    ...
    def __init__(
        ...
        self.spatial_convs = nn.ModuleList(
            DiffConv(
                in_channels=output_size, out_channels=output_size, k=spatial_kernel_size
            )
            for _ in range(spatial_convs)
        )
        self.spatial_norms = nn.ModuleList(
            Norm(norm_type=norm, in_channels=output_size) for _ in range(spatial_convs)
        )
