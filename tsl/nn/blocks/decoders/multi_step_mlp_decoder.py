...
class MultiHorizonMLPDecoder(nn.Module):
    ...
    def __init__(
        ...
        self.local_mlp = MLP(
            input_size=exog_size + 2 * context_size,
            hidden_size=hidden_size,
    ...
    def forward(self, x: torch.Tensor, u: torch.Tensor):
        ...
        global_context = repeat(global_context, "b n f -> b n h f", h=self.horizon)
        contexts = rearrange(
            contexts, "b n (h f) -> b n h f", f=self.d_context, h=self.horizon
        )
