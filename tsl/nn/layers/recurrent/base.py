...
class GRUCellBase(RNNCellBase):
    ...
    def initialize_state(self, x) -> Tensor:
        return torch.zeros(x.size(0), self.hidden_size, dtype=x.dtype, device=x.device)
...
class GraphGRUCellBase(GRUCellBase):
    ...
    def initialize_state(self, x) -> Tensor:
        return torch.zeros(
            x.size(0), x.size(-2), self.hidden_size, dtype=x.dtype, device=x.device
        )
...
class LSTMCellBase(RNNCellBase):
    ...
    def forward(
        self, x: Tensor, hc: Tuple[Tensor, Tensor], *args, **kwargs
    ) -> Tuple[Tensor, Tensor]:
        """"""
        # x: [batch, *, channels]
