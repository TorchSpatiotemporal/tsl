...
class AttentionScores(MessagePassing):
    ...
    def forward(
        self, query: Tensor, key: Tensor, edge_index: Adj, edge_attr: OptTensor = None
    ):
        ...
        if self.lin_edge is not None:
            ...
            edge_attr = self.lin_edge(edge_attr).view(edge_attr.shape[:-1] + h_and_c)
        # propagate and attend
        self.propagate(edge_index, q=query, k=key, edge_attr=edge_attr, size=(n, n))
        # retrieve scores
...
class MultiHeadGraphAttention(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.lin_query = Linear(
            self.qdim, num_heads * self.head_dim, bias_initializer="zeros"
        )
        self.lin_value = Linear(
            self.vdim, num_heads * self.head_dim, bias_initializer="zeros"
        )
    ...
    def forward(
        ...
        # propagate and attend
        out = self.propagate(
            edge_index, q=query, k=key, v=value, edge_attr=edge_attr, size=(n, n)
        )
        ...
        if return_attention_weights:
            ...
            W = torch.zeros(
                (*alpha.shape[:-2], n, n), dtype=alpha.dtype, device=alpha.device
            )
