...
def test_separation_invariant_across_configs(
    window, horizon, stride, delay, min_offset
):
    dataset = _make_dt_dataset(
        window, horizon, periods=1500, stride=stride, delay=delay
    )
    splitter = AtTimeStepSplitter(
        first_val_ts=(2020, 1, 1),
        last_val_ts=(2020, 3, 1),
        first_test_ts=(2020, 7, 1),
        min_offset=min_offset,
    )
    # 'window' refuses configs whose window gap cannot cover the horizon.
    if min_offset == 'window' and dataset.samples_offset * stride < horizon:
    ...
    expected = (
        int(np.ceil(dataset.sample_span / stride))
        if min_offset == 'sample'
        else dataset.samples_offset
    )
...
def test_unknown_offset_raises():
    dataset = _make_dt_dataset(window=4, horizon=4, periods=400)
    splitter = AtTimeStepSplitter(
        first_val_ts=(2019, 9, 1), first_test_ts=(2019, 11, 1), min_offset='nope'
    )
...
def test_explicit_non_overlapping_train_range():
    dataset = _make_dt_dataset(window=4, horizon=4, periods=400)
    idxs = AtTimeStepSplitter(
        first_train_ts=(2019, 1, 1),
...
def test_first_train_ts_only_trims_start():
    dataset = _make_dt_dataset(window=4, horizon=4, periods=400)
    full = AtTimeStepSplitter(
        first_val_ts=(2019, 9, 1),
        last_val_ts=(2019, 10, 1),
        first_test_ts=(2019, 11, 1),
    ).split(dataset)
    trimmed = AtTimeStepSplitter(
        first_train_ts=(2019, 3, 1),
        first_val_ts=(2019, 9, 1),
        last_val_ts=(2019, 10, 1),
        first_test_ts=(2019, 11, 1),
    ).split(dataset)
...
def test_only_test_specified():
    ...
    f_tr, f_te = _footprint(dataset, idxs['train']), _footprint(dataset, idxs['test'])
    assert f_tr.isdisjoint(f_te)
...
def test_only_val_specified():
    ...
    dataset = _make_dt_dataset(window=4, horizon=4, periods=400)
    idxs = AtTimeStepSplitter(
        first_val_ts=(2019, 9, 1), last_val_ts=(2019, 10, 1)
    ).split(dataset)
    assert len(idxs['test']) == 0
    assert len(idxs['train']) and len(idxs['val'])
    f_tr, f_va = _footprint(dataset, idxs['train']), _footprint(dataset, idxs['val'])
    assert f_tr.isdisjoint(f_va)
...
def test_open_ended_val_inferred_from_test():
    ...
    dataset = _make_dt_dataset(window=4, horizon=4, periods=400)
    idxs = AtTimeStepSplitter(
        first_val_ts=(2019, 9, 1), first_test_ts=(2019, 11, 1)
    ).split(dataset)
    tr, va, te = idxs['train'], idxs['val'], idxs['test']
...
def test_open_started_test_inferred_from_val():
    ...
    dataset = _make_dt_dataset(window=4, horizon=4, periods=400)
    idxs = AtTimeStepSplitter(
        first_val_ts=(2019, 9, 1), last_val_ts=(2019, 10, 1), last_test_ts=(2019, 12, 1)
    ).split(dataset)
    tr, va, te = idxs['train'], idxs['val'], idxs['test']
    assert all(len(s) for s in (tr, va, te))
    offset = int(np.ceil(dataset.sample_span / dataset.stride))
...
def test_non_datetime_index_raises():
    dataset = SpatioTemporalDataset(
        target=np.arange(400, dtype='float32'), window=4, horizon=4
    )
    splitter = AtTimeStepSplitter(
        first_val_ts=(2019, 9, 1), first_test_ts=(2019, 11, 1)
    )
