...
def test_mask_float_raises():
    ...
    with pytest.raises(RuntimeError):
        SpatioTemporalDataset(target=target, mask=np.ones((20, 3, 2), dtype='float32'))


def test_mask_wrong_temporal_dim_raises():
    target = _grid_target(20, 3, 2)
    with pytest.raises(ValueError):
        SpatioTemporalDataset(target=target, mask=np.ones((21, 3, 2), dtype=bool))
...
def test_mask_wrong_node_dim_raises():
    ...
    target = _grid_target(20, 3, 2)
    with pytest.raises(ValueError):
        SpatioTemporalDataset(target=target, mask=np.ones((20, 5, 2), dtype=bool))


def test_mask_broadcasts_over_channels():
    target = _grid_target(20, 3, 2)
    ds = SpatioTemporalDataset(
        target=target, mask=np.ones((20, 3, 1), dtype=bool), window=4, horizon=2
    )
...
def test_mask_aligned_to_horizon_not_window():
    ...
    ds = SpatioTemporalDataset(target=target, mask=mask, window=4, horizon=3, delay=1)
    item = ds[0]
