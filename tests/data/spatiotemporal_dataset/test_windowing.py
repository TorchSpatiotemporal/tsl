...
@windowing_cases
def test_windowing_properties(window, horizon, stride, delay, window_lag, horizon_lag):
...
@windowing_cases
def test_lag_subsampling_counts(
    ...
    if window > 0:
        assert len(ds._window_range) == len(ref_window_range(window, window_lag))


@windowing_cases
def test_window_horizon_disjoint_when_delay_nonneg(
    window, horizon, stride, delay, window_lag, horizon_lag
):
...
@windowing_cases
def test_window_horizon_value_disjoint_when_delay_nonneg(
    window, horizon, stride, delay, window_lag, horizon_lag
):
    ...
    if window == 0 or delay < 0:
        pytest.skip('overlap only relevant for window>0, delay>=0')
    ds, _ = _make_grid_dataset(window, horizon, stride, delay, window_lag, horizon_lag)
...
def test_window_horizon_overlap_allowed_when_delay_negative(window, horizon, delay):
    ...
    expanded = ds.expand_indices()
    overlap_seen = False
    for w_row, h_row in zip(expanded['window'].numpy(), expanded['horizon'].numpy()):
...
@pytest.mark.parametrize('window,delay', [(0, -1), (1, -2), (4, -5), (12, -13)])
