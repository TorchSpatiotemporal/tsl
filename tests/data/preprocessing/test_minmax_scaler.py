...
from .helpers import constant_feature_target, masked_target, random_target, ref_minmax
...
def test_custom_range_params_match_reference():
    x = random_target((50, 3, 2))
    sc = MinMaxScaler(out_range=(-1.0, 1.0)).fit(x)
    bias, scale = ref_minmax(x, axis=0, out_range=(-1.0, 1.0))
...
def test_invalid_range_raises():
    ...
    with pytest.raises(ValueError):
        MinMaxScaler(out_range=(1.0, 0.0)).fit(x)
    with pytest.raises(ValueError):
        MinMaxScaler(out_range=(1.0, 1.0)).fit(x)
