...
sklearn fits its scalers per-feature over the sample (row) axis, so the data here is 2D
``(samples, features)`` and the tsl scalers use ``axis=0`` to match that convention.
tsl divides by ``scale + tsl.epsilon`` while sklearn divides by ``scale``, hence the
...
from tsl.data.preprocessing.scalers import MinMaxScaler, RobustScaler, StandardScaler
...
def test_standard_scaler_matches_sklearn(data):
    ...
    np.testing.assert_allclose(tsl_sc.transform(data), sk_sc.transform(data), atol=ATOL)


@pytest.mark.parametrize('out_range', [(0.0, 1.0), (-1.0, 1.0), (0.0, 5.0)])
