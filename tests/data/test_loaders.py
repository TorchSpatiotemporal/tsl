...
def _graph(n_nodes, t=4, c=2):
    ...
    edge_index = torch.stack(
        [torch.arange(n_nodes), (torch.arange(n_nodes) + 1) % n_nodes]
    )
    return Data(
        input={'x': torch.randn(t, n_nodes, c)},
...
def test_static_loader_shuffle_changes_order_not_content():
    ...
    ordered = torch.cat(
        [first(b) for b in StaticGraphLoader(ds, batch_size=8, shuffle=False)]
    )
    torch.manual_seed(0)
    shuffled = torch.cat(
        [first(b) for b in StaticGraphLoader(ds, batch_size=8, shuffle=True)]
    )
...
def test_disjoint_loader_force_batch_adds_dim():
    ...
    forced = next(iter(DisjointGraphLoader(graphs, batch_size=3, force_batch=True)))
...
def test_disjoint_loader_exclude_keys():
    graphs = [_graph(3), _graph(5), _graph(4)]
    batch = next(iter(DisjointGraphLoader(graphs, batch_size=3, exclude_keys=['y'])))
