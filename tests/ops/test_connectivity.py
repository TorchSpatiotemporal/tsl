...
edge_index, edge_weight = parse_connectivity((edge_index, edge_weight), 'edge_index', num_nodes=num_nodes)
...
def test_infer_backend():
    ...
    x = SparseTensor(row=torch.tensor([0, 1]), col=torch.tensor([1, 0]), value=torch.tensor([1, 2]))
...
def test_convert_torch_connectivity():
    ...
    converted = convert_torch_connectivity(edge_index_attr, 'sparse', input_layout='edge_index', num_nodes=4)
    assert isinstance(converted, SparseTensor)
    assert torch.allclose(converted.to_dense(), dense)

    # Test edge_index -> dense
    converted = convert_torch_connectivity(edge_index_attr, 'dense', input_layout='edge_index', num_nodes=4)
    assert torch.allclose(converted, dense)

    # Test invalid input_layout
    try:
        convert_torch_connectivity(edge_index_attr, 'sparse', input_layout='invalid', num_nodes=5)
    ...
    try:
        convert_torch_connectivity(edge_index_attr, 'invalid', input_layout='edge_index', num_nodes=5)
    except AssertionError:
        pass
    else:
        raise AssertionError("Expected an AssertionError")
...
def test_adj_to_edge_index():
    ...
    expected_index = torch.tensor([[0, 1, 1, 2], [1, 0, 2, 1]], dtype=torch.long)
    ...
    expected_index = torch.tensor([[0, 1, 1, 2, 3, 4, 4, 5], [1, 0, 2, 1, 4, 3, 5, 4]], dtype=torch.long)
    expected_attr = torch.tensor([1., 1., 1., 1., 1., 1., 1., 1.], dtype=torch.float)
    ...
    # Test for converting a batch of adjacency matrices to edge index with backend set to numpy
