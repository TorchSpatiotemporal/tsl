...
def test_diff_conv():
    ...
    edge_index = torch.tensor([[0, 0, 0, 1, 1, 2, 2, 3],
                               [1, 2, 3, 0, 2, 0, 3, 0]])
    adj_bin = convert_torch_connectivity(edge_index, 'sparse',
                                         num_nodes=n)

    edge_weight = torch.rand(edge_index.size(1))
    adj = convert_torch_connectivity((edge_index, edge_weight), 'sparse',
                                     num_nodes=n)
    ...
    for root_weight in [True, False]:
        for add_backward in [True, False]:
            conv = DiffConv(in_channels=f, out_channels=out_f, k=k,
