...
def test_diff_conv():
    ...
    edge_index = torch.tensor([[0, 0, 0, 1, 1, 2, 2, 3], [1, 2, 3, 0, 2, 0, 3, 0]])
    adj_bin = convert_torch_connectivity(edge_index, "sparse", num_nodes=n)

    edge_weight = torch.rand(edge_index.size(1))
    adj = convert_torch_connectivity((edge_index, edge_weight), "sparse", num_nodes=n)
...
def test_normalization():
    ...
    for add_backward in [True, False]:
        ...
        with torch.no_grad():
            conv.filters.weight.data.fill_(1.0)
        ...
        assert torch.allclose(ew_norm, torch.tensor([0.5, 0.3, 0.2]))
