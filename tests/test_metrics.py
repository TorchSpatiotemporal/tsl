...
DELTA = 1e-6
x = 1.0 + torch.rand((2, 8, 2, 2), dtype=torch.float32)
y = 1.0 + torch.rand((2, 8, 2, 4), dtype=torch.float32)
...
def test_mae_masked_metric():
    y_hat_, y_, mask_ = torch_to_numpy(y_hat), torch_to_numpy(y), torch_to_numpy(mask)
...
def test_smape_metric():
    ...
    assert np.isclose(metrics_res['test_smape'], res, atol=DELTA)
...
def test_smape_masked_metric():
    y_hat_, y_, mask_ = torch_to_numpy(y_hat), torch_to_numpy(y), torch_to_numpy(mask)
    res = npf.smape(y_hat_, y_, mask_.astype(bool))
    assert np.isclose(masked_metrics_res['test_smape'], res, atol=DELTA)
...
def test_smape_masked_functional():
    y_hat_, y_, mask_ = y_hat.clone(), y.clone(), mask.clone()
    res_np = npf.smape(
        framearray_to_numpy(y_hat_), framearray_to_numpy(y_), framearray_to_numpy(mask_)
    )
